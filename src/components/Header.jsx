      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex items-center  justify-between h-16 md:h-20">
        ...
        <nav className=' lg:flex  hidden space-x-8'>
          {['Home', 'About', 'Projects', 'Experience', 'Contact'].map((item,index) => (
             ...
             className='relative text-gray-800 dark:text-gray-200 hover:violet-600 dark:hover:text-violet-400 font-medium transition-colors duration-300 group' href="#">
             {item}
              <span className='absolute bottom-0 right-0.5 left-0.5 w-0 h-0.5 bg-violet-600 group-hover:w-full transition-all duration-300' ></span>
            </motion.a>
          ))}
        </nav>
   ...
   className='mt-4 block w-full px-4 py-2 rounded-lg bg-gradient-to-r from-violet-600 to-violet-400 font-bold'>
          ...
          {contactFormOpen && (
            ...
            >
             ...
             className='bg-white dark:bg-gray-800 rounded-x1 shadow-x1 w-full max-w-md p-6'
             >
                ...
                <button
                 onClick={closeContactForm}>
                  <FiX className='w-5 h-5 text-gray-300 font-extra-bold'/>
                  </button> 
                </div>  
                {/* Input forms */}
